...
<?php $__env->startSection('title', $product->seo_title ?? $product->name . ' - ' . site('store_name')); ?>
<?php $__env->startSection('description', $product->seo_description ?? $product->short_description ?? Str::limit(strip_tags($product->description), 160)); ?>
...
<div class="bg-gray-50 product-detail"
     ...
     x-init='init(<?php echo json_encode($product->id, 15, 512) ?>, <?php echo json_encode($product->name, 15, 512) ?>, <?php echo json_encode($product->price, 15, 512) ?>, <?php echo json_encode($product->final_price, 15, 512) ?>, <?php echo json_encode($product->sale_price, 15, 512) ?>, <?php echo json_encode($imageList, 15, 512) ?>, <?php echo json_encode($product->stock, 15, 512) ?>, <?php echo json_encode($product->sku, 15, 512) ?>, <?php echo json_encode($product->weight ?? 0, 15, 512) ?>, <?php echo json_encode($countries, 15, 512) ?>, <?php echo json_encode($defaultCountry, 15, 512) ?>, <?php echo json_encode($defaultState, 15, 512) ?>, <?php echo json_encode($countrySymbol, 15, 512) ?>, <?php echo json_encode($authUserData, 15, 512) ?>)'>

    
    <div class="bg-white border-b">
        <div class="container-app py-3">
            <nav class="flex items-center gap-2 text-sm text-gray-500 flex-wrap">
                <a href="<?php echo e(route('home')); ?>" class="hover:text-blue-600 flex items-center gap-1">
                    <span class="material-symbols-outlined ml-1">home</span>الرئيسية
                </a>
                <span class="material-symbols-outlined text-xs">chevron_right</span>
                <a href="<?php echo e(route('shop.index')); ?>" class="hover:text-blue-600">المتجر</a>
                <?php if($product->category): ?>
                    <span class="material-symbols-outlined text-xs">chevron_right</span>
                    <a href="<?php echo e(route('shop.category', $product->category->slug)); ?>" class="hover:text-blue-600">
                        <?php echo e($product->category->name); ?>

                    </a>
                <?php endif; ?>
                <span class="material-symbols-outlined text-xs">chevron_right</span>
                <span class="text-gray-700 font-semibold truncate max-w-[200px]"><?php echo e($product->name); ?></span>
            </nav>
        </div>
    </div>

    <div class="container-app py-6 md:py-8">
        <form method="POST" action="<?php echo e(route('instant.submit')); ?>" @submit.prevent="submitForm($event)" id="instant-buy-form" class="grid lg:grid-cols-3 gap-6">
            <?php echo csrf_field(); ?>
            ...
            <div class="lg:col-span-2 space-y-5">
                ...
                <div class="bg-white rounded-2xl shadow-sm overflow-hidden">
                    <div class="grid md:grid-cols-2 gap-0">
                        ...
                        <div class="p-5 md:p-6 flex flex-col">
                            ...
                            <h1 class="text-2xl md:text-3xl font-extrabold text-gray-900 leading-tight mb-3"><?php echo e($product->name); ?></h1>
                            ...
                            <div class="flex items-center gap-2 mb-4">
                                ...
                                <span class="text-xs text-gray-500">(<?php echo e($product->reviews->count() ?? 0); ?> تقييم)</span>
                            </div>
                            ...
                            <div class="mb-4 last:mb-0">
                                <label class="block text-sm font-semibold text-gray-700 mb-2 flex items-center gap-2">
                                    ...
                                    <span class="text-xs text-gray-500 font-normal"
                                          x-show="selectedOptions[<?php echo e($option->id); ?>] && getOptionAdjustment(<?php echo e($option->id); ?>) !== 0"
                                          x-transition>
                                        (<span x-text="(getOptionAdjustment(<?php echo e($option->id); ?>) > 0 ? '+' : '') + formatMoney(getOptionAdjustment(<?php echo e($option->id); ?>))"></span>)
                                    </span>
                                </label>
                                <div class="flex flex-wrap gap-2">
                                    <?php $__currentLoopData = $option->values; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $value): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                        <label class="cursor-pointer">
                                            <input type="radio" name="options[<?php echo e($option->id); ?>]" value="<?php echo e($value->id); ?>"
                                                   @change="selectedOptions[<?php echo e($option->id); ?>] = $event.target.value; recalculate()"
                                                   :checked="String(selectedOptions[<?php echo e($option->id); ?>]) === '<?php echo e($value->id); ?>'"
                                                   class="peer sr-only">
                                            <span class="inline-block px-4 py-2 border-2 border-gray-200 rounded-lg text-sm font-semibold transition
                                                ...
                                                <?php if($value->price_adjustment > 0): ?>
                                                    <span class="text-xs text-gray-500">(+<?php echo e(number_format($value->price_adjustment, 0)); ?>)</span>
                                                <?php endif; ?>
                                            </span>
                                        </label>
                ...
                <?php if($hasCustomFields): ?>
                    <div class="bg-white rounded-2xl shadow-sm p-5">
                        ...
                        <?php $__currentLoopData = $product->customFields; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $field): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <div class="mb-4 last:mb-0">
                                <label class="block text-sm font-semibold text-gray-700 mb-2">
                                    ...
                                    <span class="text-xs text-gray-500 font-normal"
                                          x-show="<?php echo e((float) $field->price_effect); ?> > 0">
                                        (+<span x-text="formatMoney(<?php echo e((float) $field->price_effect); ?>)"></span>)
                                    </span>
                                </label>
                                <?php if($field->type === 'text'): ?>
                                    <input type="text" name="custom_text" x-model="customText" @input="recalculate()"
                                           ...
                                           placeholder="<?php echo e($field->placeholder ?? 'أدخل ' . $field->name); ?>">
                ...
                <div class="bg-white rounded-2xl shadow-sm p-5" x-data="{ tab: 'description' }">
                    <div class="flex border-b mb-4">
                        ...
                        <button type="button" @click="tab = 'reviews'"
                                ...
                                class="px-4 py-2 font-semibold border-b-2 transition flex items-center gap-1">
                            <span class="material-symbols-outlined text-xs">star</span> التقييمات (<?php echo e($product->reviews->count() ?? 0); ?>)
                        </button>
            ...
            <div class="lg:col-span-1">
                <div class="sticky top-4 space-y-4">
                            ...
                            <div>
                                ...
                                <select name="country_code" required x-model="countryCode" @change="onCountryChange()"
                                    ...
                                    <?php $__currentLoopData = $countries; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $code => $info): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                        <option value="<?php echo e($code); ?>"><?php echo e($info['flag'] ?? ''); ?> <?php echo e($info['name']); ?> - <?php echo e($info['name_en']); ?></option>
                                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                </select>
                            </div>
                            <div>
                    ...
                    <?php if(isset($shippingCompanies) && $shippingCompanies->count() > 0): ?>
                        <div class="bg-white rounded-2xl shadow-sm p-5">
                            <h2 class="text-base font-bold mb-3 flex items-center gap-2 text-gray-800">
        ...
        <div class="container-app py-8 border-t">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                <?php $__currentLoopData = $relatedProducts; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $rel): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <?php echo $__env->make('frontend.partials.product-card', ['product' => $rel, 'symbol' => currentCurrencySymbol()], array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
            </div>
        </div>
    <?php endif; ?>
...
function instantBuyForm() {
    return {
        ...
        // === Options ===
        <?php if($hasOptions): ?>
        selectedOptions: <?php echo json_encode($product->options->mapWithKeys(fn($o) => [$o->id => null])->toArray()); ?>,
        optionsAdjustments: <?php echo json_encode($product->options->reduce(function ($carry, $o) { foreach ($o->values as $v) { $carry[(int) $v->id] = (float) $v->price_adjustment; } return $carry; }, [])); ?>,
        <?php else: ?>
        selectedOptions: {},
        optionsAdjustments: {},
        <?php endif; ?>
        <?php if($hasCustomFields): ?>
        customFieldPrice: <?php echo e((float) ($product->customFields->whereIn('type', ['text', 'textarea'])->first()?->price_effect ?? 0)); ?>,
        <?php else: ?>
        customFieldPrice: 0,
        <?php endif; ?>
        customText: '',
        ...
        init(id, name, basePrice, finalPrice, salePrice, images, stock, sku, weight, countries, defaultCountry, defaultState, defaultSymbol, authUser) {
            ...
            <?php if($product->category): ?>
            this.product.categoryName = <?php echo json_encode($product->category->name, 15, 512) ?>;
            this.product.categorySlug = <?php echo json_encode($product->category->slug, 15, 512) ?>;
            <?php endif; ?>
            // Pre-fill user data
        ...
        async sendCalculate() {
            ...
            try {
                ...
                const res = await fetch('<?php echo e(route('instant.calculate')); ?>', {
                    method: 'POST',
                    headers: {
                ...
                if (data.success) {
                    ...
                    if (this.shippingMethod === 'standard') {
                        ...
                        const expressRes = await fetch('<?php echo e(route('instant.calculate')); ?>', {
                            method: 'POST',
                        ...
                        const standardRes = await fetch('<?php echo e(route('instant.calculate')); ?>', {
                            method: 'POST',
                            headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': document.querySelector('meta[name=csrf-token]').content, 'Accept': 'application/json', 'X-Requested-With': 'XMLHttpRequest' },
        ...
        async applyCoupon() {
            ...
            try {
                const res = await fetch('<?php echo e(route('instant.coupon')); ?>', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': document.querySelector('meta[name=csrf-token]').content,
...
<?php echo $__env->make('frontend.layout', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\amarn\ecommerce\resources\views/frontend/shop/show.blade.php ENDPATH**/ ?>