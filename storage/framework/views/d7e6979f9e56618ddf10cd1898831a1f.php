...
<?php $__env->startSection('page_title', 'إدارة العملات'); ?>
...
<div class="mb-6">
    <nav class="flex items-center gap-2 text-outline font-label-sm mb-3">
        <a href="<?php echo e(route('admin.dashboard')); ?>" class="hover:text-primary transition-colors">الرئيسية</a>
        <span class="material-symbols-outlined text-[16px]">chevron_left</span>
        <a href="<?php echo e(route('admin.settings.index')); ?>" class="hover:text-primary transition-colors">إعدادات المتجر</a>
        <span class="material-symbols-outlined text-[16px]">chevron_left</span>
...
<div class="grid grid-cols-12 gap-6">
                ...
                <table class="w-full text-right">
                    ...
                    <tbody class="font-body-sm">
                        <?php $__currentLoopData = $currencies; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $cur): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <tr class="border-b border-outline-variant hover:bg-surface-container transition-colors <?php echo e($cur['code'] === $storeCurrency ? 'bg-primary-fixed/20' : ''); ?>">
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-3">
                                        <span class="w-8 h-6 bg-surface-container-high rounded-sm flex items-center justify-center font-bold text-[10px]"><?php echo e($cur['code']); ?></span>
                                        <span class="font-medium <?php echo e($cur['code'] === $storeCurrency ? 'text-primary' : ''); ?>">
                                            <?php echo e($cur['country_name']); ?>

                                            <?php if($cur['code'] === $storeCurrency): ?>
                                                <span class="text-primary-fixed-dim text-xs mr-1">(الأساسية)</span>
                                            <?php endif; ?>
                                        </span>
                                    </div>
                                </td>
                                <td class="px-6 py-4 text-on-surface-variant font-bold"><?php echo e($cur['symbol']); ?></td>
                                <td class="px-6 py-4 font-mono"><?php echo e(number_format($cur['rate_to_usd'] ?? 1, 4)); ?></td>
                                <td class="px-6 py-4">
                                    <?php if($cur['code'] === $storeCurrency): ?>
    ...
    <div class="col-span-12 lg:col-span-4 space-y-6">
        ...
        <div class="bg-surface-container-lowest rounded-lg border border-outline-variant shadow-sm p-6">
            ...
            <form method="POST" action="<?php echo e(route('admin.currencies.update')); ?>" class="space-y-4">
                <?php echo csrf_field(); ?>

                <div class="flex flex-col gap-1.5">
                    <label class="font-label-md text-on-surface">الدولة الافتراضية <span class="text-error">*</span></label>
                    <select name="default_country" required class="bg-surface-container-low border border-outline-variant p-2.5 rounded font-body-sm focus:ring-2 focus:ring-primary outline-none <?php $__errorArgs = ['default_country'];
...
unset($__errorArgs, $__bag); ?>">
                        ...
                        ?>
                        <?php $__currentLoopData = $supported; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $code => $info): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <option value="<?php echo e($code); ?>" <?php echo e(old('default_country', $defaultCountry) === $code ? 'selected' : ''); ?>>
                                <?php echo e($info['name']); ?> (<?php echo e($code); ?>)
                            </option>
                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                    </select>
                ...
                <div class="flex flex-col gap-1.5">
                    ...
                    <input type="text" name="currency" required maxlength="3" minlength="3"
                           value="<?php echo e(old('currency', $storeCurrency)); ?>"
                           placeholder="SDG, EGP, USD, EUR..."
                           class="bg-surface-container-low border border-outline-variant p-2.5 rounded font-body-sm focus:ring-2 focus:ring-primary outline-none font-mono uppercase <?php $__errorArgs = ['currency'];
$__bag = $errors->getBag($__errorArgs[1] ?? 'default');
if ($__bag->has($__errorArgs[0])) :
if (isset($message)) { $__messageOriginal = $message; }
$message = $__bag->first($__errorArgs[0]); ?> border-error <?php unset($message);
...
unset($__errorArgs, $__bag); ?>">
                ...
                <div class="flex flex-col gap-1.5">
                    ...
                    <input type="text" name="currency_symbol" required maxlength="10"
                           value="<?php echo e(old('currency_symbol', $storeSymbol)); ?>"
                           placeholder="ج.س, $, €, £..."
                           class="bg-surface-container-low border border-outline-variant p-2.5 rounded font-body-sm focus:ring-2 focus:ring-primary outline-none <?php $__errorArgs = ['currency_symbol'];
$__bag = $errors->getBag($__errorArgs[1] ?? 'default');
if ($__bag->has($__errorArgs[0])) :
if (isset($message)) { $__messageOriginal = $message; }
$message = $__bag->first($__errorArgs[0]); ?> border-error <?php unset($message);
        ...
        <div class="bg-surface-container-lowest rounded-lg border border-outline-variant shadow-sm p-6 overflow-hidden relative">
            ...
            <div class="space-y-3">
                ...
                <div class="flex items-center gap-2 text-on-surface-variant">
                    ...
                    <span class="font-label-sm">آخر تحديث: <?php echo e(now()->format('Y-m-d h:i A')); ?></span>
                </div>
            </div>
        </div>
    </div>
            ...
            <tbody>
                <?php $__currentLoopData = $currencies; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $cur): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <tr class="border-b border-outline-variant hover:bg-surface-container transition-colors <?php echo e($cur['code'] === $storeCurrency ? 'bg-primary-fixed/20' : ''); ?>">
                        ...
                        <td class="px-6 py-4 font-mono font-bold"><?php echo e($cur['code']); ?></td>
                        <td class="px-6 py-4"><?php echo e($cur['country_name']); ?> <span class="text-xs text-on-surface-variant">(<?php echo e($cur['country']); ?>)</span></td>
                        <td class="px-6 py-4 font-mono"><?php echo e($cur['dial_code']); ?></td>
                        <td class="px-6 py-4 text-xs text-on-surface-variant font-mono">1 USD ≈ <?php echo e(number_format($cur['rate_to_usd'] ?? 1, 2)); ?> <?php echo e($cur['code']); ?></td>
                        <td class="px-6 py-4">
                            <?php if($cur['code'] === $storeCurrency): ?>
...
<?php echo $__env->make('admin.layout', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\amarn\ecommerce\resources\views/admin/currencies/index.blade.php ENDPATH**/ ?>