@if(site('top_bar_show', '1') === '1')
    ...
    @endphp
    {{-- Top promo bar --}}
    <div class="text-xs py-2 shadow-sm relative z-50 transition-all duration-200" style="background-color: {{ $topBarBg }}; color: {{ $topBarColor }}">
        <div class="container-app flex items-center justify-between flex-wrap gap-2">
            <div class="flex items-center gap-4 flex-wrap">
                @if($topBarLink)
                    <a href="{{ $topBarLink }}" class="flex items-center gap-1.5 opacity-90 hover:opacity-100 transition-opacity">
            ...
            <div class="flex items-center gap-4">
                ...
                <span class="opacity-35 text-xs">|</span>
                <a href="{{ route('track') }}" class="hover:opacity-80 transition text-xs" style="color: {{ $topBarColor }}">تتبع طلبك</a>
                <span class="opacity-35 text-xs">|</span>
                <a href="{{ route('page.show', 'faq') }}" class="hover:opacity-80 transition text-xs" style="color: {{ $topBarColor }}">المساعدة</a>
            </div>
        </div>
    </div>
@endif
