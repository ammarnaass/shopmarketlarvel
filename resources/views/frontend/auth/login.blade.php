...
<section class="min-h-[85vh] flex items-center justify-center py-12 dark:from-gray-900 dark:via-gray-900 dark:to-gray-800" style="background: linear-gradient(135deg, {{ $siteSettings['primary_color'] ?? '#004ac6' }}11 0%, {{ $siteSettings['accent_color'] ?? '#f59e0b' }}11 50%, {{ $siteSettings['primary_color'] ?? '#004ac6' }}08 100%);">
    <div class="w-full max-w-md mx-auto px-4">
        <div class="card animate-fade-up overflow-hidden border border-outline-variant/60 dark:border-gray-700 shadow-xl dark:bg-gray-800/90 backdrop-blur-sm">
            {{-- Header with logo --}}
            <div class="relative overflow-hidden p-8 text-center" style="background: linear-gradient(135deg, {{ $siteSettings['primary_color'] ?? '#004ac6' }}, {{ $siteSettings['accent_color'] ?? '#f59e0b' }});">
            ...
            <div class="card-body p-6 md:p-8">
                ...
                <form method="POST" action="{{ route('login') }}" class="space-y-4">
                    ...
                    <button type="submit" class="btn-block btn-lg mt-2 text-white font-bold rounded-xl py-3.5 transition-all duration-200 hover:brightness-110 active:scale-[0.98] shadow-lg" style="background: linear-gradient(135deg, {{ $siteSettings['primary_color'] ?? '#004ac6' }}, {{ $siteSettings['accent_color'] ?? '#f59e0b' }});">
