        ...
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-xs mt-1 {{ $stats['revenue_growth'] >= 0 ? 'text-emerald-600' : 'text-error' }}">
                    ...
                    {{ $stats['revenue_growth'] >= 0 ? '+' : '' }}{{ $stats['revenue_growth'] }}% عن الشهر السابق
                </p>
            </div>
            <div class="kpi-icon">
    ...
    <div class="kpi-card">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-xs mt-1 text-on-surface-variant">+{{ $stats['new_customers_this_month'] }} هذا الشهر</p>
            </div>
            <div class="kpi-icon">
            ...
            <div>
                ...
                <p class="text-xs mt-1 {{ $stats['low_stock'] > 0 ? 'text-warning' : 'text-emerald-600' }}">
                    @if($stats['low_stock'] > 0)
                        <span class="material-symbols-outlined text-sm align-text-bottom">warning</span> {{ $stats['low_stock'] }} مخزون منخفض
                    @else
    ...
    <div class="card-body">
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div class="border border-outline-variant/50 rounded-lg p-3">
                ...
                <a href="{{ route('admin.coupons.index') }}" class="text-primary text-xs hover:underline">إعدادات</a>
            </div>
            <div class="border border-outline-variant/50 rounded-lg p-3">
...
<div class="grid lg:grid-cols-3 gap-6">
    ...
    <div class="lg:col-span-2 card">
        <div class="card-header flex items-center justify-between">
            ...
            <a href="{{ route('admin.orders.index') }}" class="text-primary text-sm hover:underline">عرض الكل</a>
        </div>
                ...
                <tbody>
                    @forelse($recentOrders as $order)
                        <tr>
                            ...
                            <td>
                                {{ $order->user?->name ?? $order->shippingAddress?->name ?? 'زائر' }}
                            ...
                            <td>
                                ...
                                @if($order->items->count() > 1)
                                    <div class="text-xs text-on-surface-variant">+{{ $order->items->count() - 1 }} منتج</div>
                                @endif
                            </td>
    ...
    <div class="space-y-6">
        ...
        <div class="card">
            <div class="card-header flex items-center justify-between">
                ...
                <a href="{{ route('admin.products.index') }}" class="text-primary text-xs hover:underline">الكل</a>
            </div>
            <div class="divide-y divide-outline-variant/30">
                @forelse($lowStockProducts as $p)
                    <div class="p-3 flex items-center gap-3">
                        ...
                        <div class="text-left">
                            <span class="badge badge-danger">{{ $p->stock }} قطع</span>
                        </div>
                    </div>
                @empty
                ...
                @forelse($topProducts as $i => $tp)
                    <div class="p-3 flex items-center gap-3">
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <div class="text-xs text-on-surface-variant">{{ $tp->total_qty }} قطعة مباعة</div>
                        </div>
        ...
        <div class="card">
            <div class="card-header flex items-center justify-between">
                ...
                <a href="{{ route('admin.coupons.index') }}" class="text-primary text-xs hover:underline">إدارة</a>
            </div>
            <div class="divide-y divide-outline-variant/30">
