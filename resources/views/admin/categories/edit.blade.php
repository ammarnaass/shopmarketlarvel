...
@section('content')
<div class="mb-6">
    <h1 class="text-3xl font-bold">{{ __t('common.edit') }} {{ __t('admin.categories.title') }}</h1>
...
<form method="POST" action="{{ route('admin.categories.update', $category) }}" enctype="multipart/form-data">
        ...
        <div class="lg:col-span-2 space-y-6">
            <div class="bg-surface-container-lowest rounded-xl shadow-sm p-5">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="icon" value="{{ old('icon', $category->icon) }}" class="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 @error('icon') border-red-500 @enderror" placeholder="{{ __t('admin.categories.icon_placeholder') }}">
    ...
    <div class="mt-6 flex items-center gap-3">
        <button type="submit" class="bg-primary hover:bg-primary text-white px-6 py-2.5 rounded-lg font-semibold flex items-center gap-2">
            <span class="material-symbols-outlined">save</span>{{ __t('common.update') }}
        </button>
