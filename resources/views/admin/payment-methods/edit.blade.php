...
@section('title', __t('admin.payment_methods.edit_method'))
...
<div class="mb-6">
    ...
    <h1 class="text-3xl font-bold mt-2">{{ __t('admin.payment_methods.edit_method') }}</h1>
</div>
...
<form method="POST" action="{{ route('admin.payment-methods.update', $method) }}">
    ...
    <div class="bg-surface-container-lowest rounded-xl shadow-sm p-5 mb-6">
        ...
        <div class="grid md:grid-cols-2 gap-4">
            <div>
                ...
                <input type="text" name="name" value="{{ old('name', $method->name) }}" required class="w-full px-3 py-2 border border-outline-variant rounded-lg focus:ring-2 focus:ring-primary @error('name') border-red-500 @enderror">
            ...
            <div>
                ...
                <input type="text" name="code" value="{{ old('code', $method->code) }}" required class="w-full px-3 py-2 border border-outline-variant rounded-lg focus:ring-2 focus:ring-primary font-mono text-sm" dir="ltr">
            ...
            <div>
                ...
                <select name="type" required class="w-full px-3 py-2 border border-outline-variant rounded-lg focus:ring-2 focus:ring-primary">
                    <option value="manual" {{ old('type', $method->type) === 'manual' ? 'selected' : '' }}>{{ __t('admin.payment_methods.type_manual') }}</option>
                    <option value="gateway" {{ old('type', $method->type) === 'gateway' ? 'selected' : '' }}>{{ __t('admin.payment_methods.type_gateway') }}</option>
                    <option value="wallet" {{ old('type', $method->type) === 'wallet' ? 'selected' : '' }}>{{ __t('admin.payment_methods.type_wallet') }}</option>
                </select>
            </div>
            <div>
                <label class="block text-sm font-semibold mb-1">{{ __t('admin.payment_methods.sort_order') }}</label>
                <input type="number" name="sort_order" value="{{ old('sort_order', $method->sort_order) }}" min="0" class="w-full px-3 py-2 border border-outline-variant rounded-lg focus:ring-2 focus:ring-primary">
            </div>
            <div class="md:col-span-2">
                <label class="block text-sm font-semibold mb-1">{{ __t('admin.payment_methods.description') }}</label>
                <textarea name="description" rows="2" class="w-full px-3 py-2 border border-outline-variant rounded-lg focus:ring-2 focus:ring-primary">{{ old('description', $method->description) }}</textarea>
            </div>
        </div>
    </div>
    ...
    <div class="bg-surface-container-lowest rounded-xl shadow-sm p-5 mb-6">
        ...
        <div class="grid md:grid-cols-2 gap-4">
            <div>
                ...
                <input type="text" name="icon" value="{{ old('icon', $method->icon) }}" required class="w-full px-3 py-2 border border-outline-variant rounded-lg focus:ring-2 focus:ring-primary font-mono text-sm" dir="ltr">
            ...
            <div>
                ...
                <select name="color" required class="w-full px-3 py-2 border border-outline-variant rounded-lg focus:ring-2 focus:ring-primary">
                    @foreach(['green','blue','purple','orange','red','indigo','teal','pink','cyan','yellow'] as $c)
                        <option value="{{ $c }}" {{ old('color', $method->color) === $c ? 'selected' : '' }}>{{ $c }}</option>
                    @endforeach
                </select>
            </div>
        </div>
        ...
        <div class="grid md:grid-cols-3 gap-4">
            <div>
                ...
                <select name="fees_type" required class="w-full px-3 py-2 border border-outline-variant rounded-lg focus:ring-2 focus:ring-primary">
                    <option value="fixed" {{ old('fees_type', $method->fees_type) === 'fixed' ? 'selected' : '' }}>{{ __t('admin.payment_methods.fees_fixed') }}</option>
                    <option value="percent" {{ old('fees_type', $method->fees_type) === 'percent' ? 'selected' : '' }}>{{ __t('admin.payment_methods.fees_percent') }}</option>
                </select>
            </div>
            <div>
                <label class="block text-sm font-semibold mb-1">{{ __t('admin.payment_methods.fees_value') }}</label>
                <input type="number" name="fees_value" value="{{ old('fees_value', $method->fees_value) }}" min="0" step="0.01" class="w-full px-3 py-2 border border-outline-variant rounded-lg focus:ring-2 focus:ring-primary">
            </div>
            <div></div>
            <div>
                <label class="block text-sm font-semibold mb-1">{{ __t('admin.payment_methods.min_order') }}</label>
                <input type="number" name="min_order" value="{{ old('min_order', $method->min_order) }}" min="0" step="0.01" class="w-full px-3 py-2 border border-outline-variant rounded-lg focus:ring-2 focus:ring-primary">
            </div>
            <div>
                <label class="block text-sm font-semibold mb-1">{{ __t('admin.payment_methods.max_order') }}</label>
                <input type="number" name="max_order" value="{{ old('max_order', $method->max_order) }}" min="0" step="0.01" class="w-full px-3 py-2 border border-outline-variant rounded-lg focus:ring-2 focus:ring-primary" placeholder="{{ __t('admin.payment_methods.no_limit') }}">
            </div>
        </div>
    </div>
    ...
    <div class="bg-surface-container-lowest rounded-xl shadow-sm p-5 mb-6">
        ...
        <div>
            ...
            <textarea name="instructions" rows="4" class="w-full px-3 py-2 border border-outline-variant rounded-lg focus:ring-2 focus:ring-primary">{{ old('instructions', $method->instructions) }}</textarea>
    ...
    <div class="bg-surface-container-lowest rounded-xl shadow-sm p-5 mb-6">
        <label class="flex items-center gap-3 cursor-pointer">
            <input type="checkbox" name="is_active" value="1" {{ old('is_active', $method->is_active) ? 'checked' : '' }} class="w-5 h-5 text-primary rounded">
            <div>
    ...
    <div class="bg-surface-container-lowest rounded-xl shadow-sm p-5 flex items-center justify-between">
        ...
        <button type="submit" class="bg-primary hover:bg-primary text-white px-6 py-2.5 rounded-lg font-semibold flex items-center gap-2 transition active:scale-95">
            ...
            {{ __t('admin.payment_methods.save') }}
        </button>
    </div>
</form>
@endsection
