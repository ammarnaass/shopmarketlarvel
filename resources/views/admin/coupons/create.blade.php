...
@section('content')
<div class="mb-6">
    <h1 class="text-3xl font-bold">إضافة كوبون جديد</h1>
...
<form method="POST" action="{{ route('admin.coupons.store') }}">
        ...
        <div class="grid md:grid-cols-2 gap-4">
            ...
            <div>
                ...
                <select name="type" required class="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 @error('type') border-red-500 @enderror">
                    <option value="fixed" {{ old('type', 'fixed') === 'fixed' ? 'selected' : '' }}>مبلغ ثابت</option>
            ...
            <div>
                <label class="block text-sm font-semibold mb-1">الحد الأقصى للخصم (للنسبة %)</label>
    ...
    <div class="mt-6 flex items-center gap-3">
        <button type="submit" class="bg-primary hover:bg-primary text-white px-6 py-2.5 rounded-lg font-semibold flex items-center gap-2">
            <span class="material-symbols-outlined">save</span>حفظ الكوبون
        </button>
