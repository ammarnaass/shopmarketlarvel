...
<form method="POST" action="{{ $page ? route('admin.pages.update', $page) : route('admin.pages.store') }}" class="max-w-4xl">
    ...
    <div class="bg-surface-container-lowest rounded-xl shadow-sm p-6 space-y-4">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                <label class="block text-sm font-semibold text-on-surface mb-1">عنوان الصفحة *</label>
                <input type="text" name="title" value="{{ old('title', $page->title ?? '') }}" required
                       class="w-full border rounded-lg px-4 py-2.5 @error('title') border-red-500 @enderror">
                @error('title')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
            </div>
            <div>
                <label class="block text-sm font-semibold text-on-surface mb-1">الرابط المختصر *</label>
        ...
        <div>
            <label class="block text-sm font-semibold text-on-surface mb-1">محتوى الصفحة</label>
            <textarea name="content" rows="15"
                      class="w-full border rounded-lg px-4 py-2.5 font-mono text-sm">{{ old('content', $page->content ?? '') }}</textarea>
            <p class="text-gray-400 text-xs mt-1">يدعم HTML أو Markdown</p>
        </div>

        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                <label class="block text-sm font-semibold text-on-surface mb-1">عنوان SEO</label>
                <input type="text" name="meta_title" value="{{ old('meta_title', $page->meta_title ?? '') }}"
                       class="w-full border rounded-lg px-4 py-2.5 text-sm" placeholder="يُستخدم في محركات البحث">
            </div>
            <div>
                <label class="block text-sm font-semibold text-on-surface mb-1">وصف SEO</label>
        ...
        <div class="flex items-center gap-6">
            <label class="flex items-center gap-2">
                ...
                <span class="text-sm font-semibold text-on-surface">نشط</span>
            </label>
            <div class="flex items-center gap-2">
                <label class="text-sm font-semibold text-on-surface">الترتيب:</label>
        ...
        <div class="flex gap-3 pt-4 border-t">
            <button type="submit" class="bg-primary hover:bg-primary text-white px-6 py-2.5 rounded-lg font-semibold text-sm">
                <span class="material-symbols-outlined ml-1">save</span> {{ $page ? 'تحديث' : 'حفظ' }}
            </button>
